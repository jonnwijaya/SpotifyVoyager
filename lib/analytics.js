...
export const getUserAnalytics = async () => {
  try {
    ...
    // Try to get data, but handle errors gracefully for free accounts
    let topArtists = { items: [] };
    let topTracks = { items: [] };
    let recentTracks = { items: [] };

    try {
      topArtists = await spotifyFetch('/me/top/artists?limit=50&time_range=medium_term');
    } catch (error) {
      console.warn('Could not fetch top artists (may require premium):', error.message);
    }

    try {
      topTracks = await spotifyFetch('/me/top/tracks?limit=50&time_range=medium_term');
    } catch (error) {
      console.warn('Could not fetch top tracks (may require premium):', error.message);
    }

    try {
      recentTracks = await spotifyFetch('/me/player/recently-played?limit=50');
    } catch (error) {
      console.warn('Could not fetch recent tracks:', error.message);
    }

    // Get audio features for top tracks (if available)
    const audioFeatures = topTracks.items?.length > 0 ? await getTopTracksAudioFeatures() : [];

    // Generate insights with fallback data
